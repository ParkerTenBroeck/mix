...
impl<'a> ProgramBuilder for ExprBuilder<'a> {
    ...
    fn emit_lambda(&mut self, span: Span, expr: impl FnOnce(&mut ExprBuilder)) -> (LambdaId, CodeLoc) {
...
impl<'a> ByteCodeBuilder for ExprBuilder<'a>{
    ...
    fn new(&mut self) -> ExprBuilder<'_> {
        ExprBuilder { code: Default::default(), program: self.program }
    }
}
...
impl<'a, T: ByteCodeBuilder + ?Sized> ThenBuilder<'a, T>{
    pub fn emit_else(self, else_expr: impl FnOnce(&mut ExprBuilder)) -> &'a mut T{
        ...
        self.builder.emit(OpCode::If(CodeLocOffset(then_code.len())));
        for op in then_code{
